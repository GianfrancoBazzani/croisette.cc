...
# Get WETH balance
WETH_BALANCE=$(./bin/cast call $WETH "balanceOf(address)(uint256)" $WALLET --rpc-url $ETHEREUM_SEPOLIA_RPC_URL | awk '{print $1}')
...
echo "Quote response:"
echo "$QUOTE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(json.dumps(d, indent=2)[:1000])"