...
Generates a signable transaction from a quote. Used by the `swap-execution` skill after user approval — **never by `swap-preparation`**.

### Request

```json
{
  "quote": { /* the full quote response object from POST /quote */ },
...
Required headers: same as all other endpoints (`x-api-key`, `Content-Type`, `x-universal-router-version: 2.0`).
...
- `swap.to` — Universal Router contract address
- `swap.from` — managed wallet address (must match swapper)
- `swap.data` — encoded calldata for the swap
- `swap.value` — ETH value in hex (non-zero for ETH-input swaps)
- `simulationError` — `null` means simulation passed; any value means reject

### Validation before signing

Reject the transaction if:
- `simulationError` is non-null
- `swap.from` does not match the managed wallet
- `swap.chainId` does not match the expected chain
- `swap.data` is empty
- `swap.to` is missing

### Signing and broadcasting via cast

After simulation passes, execute the unsigned transaction:
...
Wait for the transaction to be mined. `cast send` blocks until confirmation and returns the tx hash.

### Execution flow (post-approval only)

1. Refresh `/check_approval` — execute approval tx if needed
2. Refresh `/quote` — get a fresh quote (proposal quotes are stale)
3. Call `/swap` with `simulateTransaction: true` — preflight
4. If simulation passes, sign and broadcast via `cast send`
5. Report tx hash and result to user

**The `swap-preparation` skill stops at step 2. Steps 3-5 are handled by `swap-execution` only.**
